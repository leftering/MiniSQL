			  ...
			  if (is_unique(table_name, col_name)){
				...
				int k;for(k = 0;k<T.col_num;k++)
				{
					if (T.col[k].col_name == col_name) { 
						...
						In.table.col[k].have_index = true;
						T.write_table_info();
						break;
					}
				}
				int create_index_result;
				ifstream test((table_name + "#" + col_name + ".tree").c_str());
				if (test.is_open())
				{
					cout << "the index tree has been builted" << endl;
					test.close();
					//drop_index(index_name);
					create_index_result = 0;
				}
				else
				{
					test.close();
		...
		if (strcmp(drop_type.c_str(), "table") == 0) {	// drop table
			...
			if (index_name != str_ERROR) {
			  //call drop index here
				...
				In.table.get_table_info(ftable_name);
				int k;for(k = 0;k<T.col_num;k++)
				{
					if (T.col[k].col_name == fattributename)
					{
						T.col[k].have_index = false;
						In.table.col[k].have_index = false;
						T.write_table_info();
					}
				}
				//drop index:
